...
Hanif ganteng✓ = [ P,M,H,K,B,U,O,N ] # warna janda x
komen = random.choice(['Mantap bang @[100000626195514:0] I Love You','Pengguna script kang coli  bang @[100000626195514:0]','Kamu tolol banget deh @[100044663564610:0]','Kamu anjing banget deh bang @[100046031671446:0]','Kamu goblok bang bang @[100056394630237:0]','Kamu  anak siapa @[100062167515808:0]','Kamu ganteng banget bang @[100076440339167:0]','Mantap bang love you','Hanif nggak ada obat script nya emang','pengin kaya elu bang bisa ngocok stading @[100000626195514:0]'])
...
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] Author   : Hanif Gamteng
 [\x1b[1;96m+%s] Github   : gd gw kang recode
 [\x1b[1;96m+%s] Facebook : gd gw kang recode
───────────────────────────────────────────────────────\n'''%(N,N,N,N))
...
class login:
	...
	def __login__(self):
		os.system('clear')
		banner()
		token = input(' [%s*%s] Masukan toket : '%(O,N))
		if toket in ['']:
			...
			try:
				...
				self.takon()
	def takon(self):
		takon = input('\n %s[%s!%s] Mau tau cara remes toket y/t: '%(N,O,N))
		if takon in ['y','Y','iya']:
			jalan('\n %s[%s!%s] Kamu akan di arahkan ke yt'%(N,O,N))
			os.system('xdg-open 'https://youtu.be/iLkq3Ah_ZS4');exit()
		else:
			login().__login__()
	def bot(self):
...
class menu:
	...
	def pilih(self):
		...
		if usna in ['']:
			print(' %s║'%(N))
			print(' %s╚═[%s!%s] Jangan kosong tolol'%(N,M,N));time.sleep(2);menu().main()
		elif usna in ['0','00']:
			try:
				token = open('token.x','r').read()
			except IOError:
				os.system('rm -rf token.x')
				exit(' %s╚═[%s!%s] Cek toket kamu'%(N,M,N))
			try:
			...
			try:
				...
				os.system('rm -rf token.x')
				exit(' %s╚═[%s!%s] Coba ulang tolol !'%(N,M,N))
			try:
				print(' %s║'%(N))
